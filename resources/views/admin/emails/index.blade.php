                                  ...
                                  <tr class="text-gray-700">
                                    <td class="px-4 py-3 border">
                                      <div class="flex items-center text-sm">
                                        <div>
                                          <p class="font-semibold text-black">{{$email->type}}</p>
                                        </div>
                                      </div>
                                    </td>
                                    <td class="px-4 py-3 text-ms font-semibold border">{{$email->email}}</td>
                                    <td class="px-4 py-3 text-xs border">
                                        <a href="{{route('admin.emails.edit',$email->id)}}">
                                            <i class="far fa-edit fa-2x text-green-500 hover:text-green-600"></i>
                                        </a>
                                        <form action="{{route('admin.emails.destroy',$email->id)}}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button>
