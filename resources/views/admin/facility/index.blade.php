        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class=" rounded overflow-hidden border w-full h-5/6 bg-white">
                    <img class="w-full h-5/6 bg-contain" src="{{asset('storage/facility/'. $image->image)}}">
                    <div class="px-3">
                      <div class="pt-2 text-center">
                          <form action="{{route('admin.facility.update',$image->id)}}" class="inline" method="POST">
                            @csrf
                            @method('PUT')
                            <button>
                            ...
                            <form action="{{route('admin.facility.destroy',$image->id)}}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button>
